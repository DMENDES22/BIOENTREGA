...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser) {
        try {
          ...
          console.error("Erro ao syncar cadastro com Firestore:", error);
  ...
  // --- 2. Real-Time Deliveries Synchronizer (Firestore snapshot list) ---
  useEffect(() => {
    if (!isRealFirebase || !currentUser) return;

    let q;
    if (currentUser.role === 'ADMIN') {
      // Admins are allowed to retrieve complete list of dispatches
      q = collection(db, 'deliveries');
    } else {
      // Drivers only query their specifically designed tracking list (Enforced in rules to avoid PERMISSION_DENIED)
      q = query(
        collection(db, 'deliveries'),
        where('assignedDriver', '==', currentUser.name)
      );
    }

    const unsubscribe = onSnapshot(q, (snapshot) => {
      const docsData: Delivery[] = [];
      snapshot.forEach((doc) => {
        docsData.push({ id: doc.id, ...doc.data() } as Delivery);
      });

      // Maintain pristine chronological order mapping (updatedAt descending)
      docsData.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      setDeliveries(docsData);
    }, (error) => {
      console.error("Erro na escuta das entregas no Firestore:", error);
    });
